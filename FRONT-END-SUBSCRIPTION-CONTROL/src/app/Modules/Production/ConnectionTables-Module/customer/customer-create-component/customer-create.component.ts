...
export class CustomerCreateComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.formulario.valid) {
      ...
      this.service.createPerson(this.formulario.value).subscribe(person => {
        var str = this.formulario.value.cpFeCNPJ.replace('-', '').replace('.', '').replace('/', '');
        var pwd = person.id.replace('-','').replace('.','').replace('/','').replace('-','').replace('-','').replace('-','');
        console.log(pwd)
        console.log(str)
        this.formulario3.patchValue({
          person_Id: person.id,
          requestSupportUsername: str,
          requestSupportPassword: pwd,
          insercionDate: this.dt
        ...
        if (this.formulario3.valid) {
          ...
          this.listCustomers();
          this.service.createCustomer(this.formulario3.value).subscribe(customer => {
  ...
  searchCnpj(cnpj: string) {
    ...
    if (this.formulario.controls['cpFeCNPJ'].valid) {
      ...
      console.log(str)
      this.service.getCustomerByCnpj(str).subscribe(cnpj => {
        if (cnpj != null) {
...
searchCpf(cpf: string) {
  ...
  if (this.formulario.controls['cpFeCNPJ'].valid) {
    ...
    this.service.getCustomerByCpf(str).subscribe(cpf => {
      console.log(cpf)
